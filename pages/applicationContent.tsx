...
import { GenerateContent, ExecuteCypher, ExecuteGoogleSearch, ExecuteCrystalKnows } from '../lib/middleware';
...
const ApplicationContent: NextPage = () => {
  ...
  const StreamResponse = async (e: any) => {
    ...
    if (googleSearch) {
      ...
      responseText = await GenerateContent(isUserDefined, prompt, false, respondWithChart, llmKey, googleSearch, crystalKnows);
    } else if (crystalKnows) {
      const prompt = prompts.CONVERT_TO_CURL_PROMPT(userInput);
      responseText = await GenerateContent(isUserDefined, prompt, true, respondWithChart, llmKey, googleSearch, crystalKnows);
    } else {
    ...
    try{
        ...
        if (!googleSearch || !crystalKnows){
          var neoResponse:any = await ExecuteCypher(isUserDefined, false,  selectedAgentKey, query, {googleSearch, crystalKnows});
        } else if (googleSearch) {
          var neoResponse:any = await ExecuteGoogleSearch(isUserDefined, false,  selectedAgentKey, query, {googleSearch, crystalKnows});
        } else if (crystalKnows){
        ...
        let finalResponse = await GenerateContent(isUserDefined, prompt, false, respondWithChart, llmKey, googleSearch, crystalKnows)
        console.log('finalResponse \n' , finalResponse);

        let finalMessage = ""
        if(!respondWithChart && !googleSearch && !crystalKnows)
        {
        ...
        let response = await GenerateContent(isUserDefined, prompt, false, false, llmKey, false, false)
