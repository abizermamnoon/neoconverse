...
import { GenerateContent, ExecuteCypher, ExecuteGoogleSearch } from '../lib/middleware';
...
const ApplicationContent: NextPage = () => {
  ...
  const StreamResponse = async (e: any) => {
    ...
    if (googleSearch) {
      ...
      responseText = await GenerateContent(isUserDefined, prompt, false, respondWithChart, llmKey, googleSearch);
    } else {
      const prompt = prompts.CYPHER_GENERATION_PROMPT(initialContext, fewShot, context, userInput);
      responseText = await GenerateContent(isUserDefined, prompt, true, respondWithChart, llmKey, googleSearch);
    }
    ...
    try{
        ...
        if (!googleSearch){
          var neoResponse:any = await ExecuteCypher(isUserDefined, false,  selectedAgentKey, query, {googleSearch});
        } else {
          var neoResponse:any = await ExecuteGoogleSearch(isUserDefined, false,  selectedAgentKey, query, {googleSearch});
        }
        ...
        let finalResponse = await GenerateContent(isUserDefined, prompt, false, respondWithChart, llmKey, googleSearch)
        console.log('finalResponse \n' , finalResponse);

        let finalMessage = ""
        if(!respondWithChart && !googleSearch)
        {
          ...
          const match = finalResponse.match(/RETURN\s+'(.+)' as response/);
          if (match && match[1]) {
        ...
        let response = await GenerateContent(isUserDefined, prompt, false, false, llmKey, false)
  ...
  async function runCypher(cypher:string){
    ...
    //  console.log('Cypher Query Generated by LLM \n' , cypher);
     ...
     if (!googleSearch){
      var neoResponse:any = await ExecuteCypher(isUserDefined, false,  selectedAgentKey, cypher, {googleSearch});
    } else {
      var neoResponse:any = await ExecuteGoogleSearch(isUserDefined, false,  selectedAgentKey, cypher, {googleSearch});
    }
    //  console.log('Result from Neo4j' , neoResponse.result);
